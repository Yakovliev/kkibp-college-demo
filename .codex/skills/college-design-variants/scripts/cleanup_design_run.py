#!/usr/bin/env python3
"""Safely remove generated college design-lab runs."""
...
def find_workspace(start: Path) -> Path:
    for candidate in (start, *start.parents):
        if (candidate / ".git").exists() and (candidate / "css" / "styles.css").exists():
            return candidate
    return start
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Safely delete a generated college design variant run."
    )
    ...
    if manifest.get("generatedBy") != "college-design-variants":
        return fail('manifest must contain "generatedBy": "college-design-variants"')
